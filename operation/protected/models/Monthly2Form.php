...
class Monthly2Form extends CFormModel
{
	public $id;
	public $year_no;
	public $month_no;
	...
	public function retrieveData($index) {
		...
		$sql = "select a.year_no, a.month_no, b.id, b.hdr_id, b.data_field, b.data_value, c.name, c.upd_type, c.field_type, c.function_name, b.manual_input, a.lcd, 
				...
				docman$suffix.countdoc('OPERB1',a.id) as operb1countdoc,
		...
		if (count($rows) > 0) {
			...
			foreach ($rows as $row) {
				if ($hid!=$row['hdr_id']) {
					...
					$this->no_of_attm['operb1'] = $row['operb1countdoc'];
					$this->no_of_attm['operb2'] = $row['operb2countdoc'];
					$this->no_of_attm['operb3'] = $row['operb3countdoc'];
				}
				$temp = array();
				$temp['id'] = $row['id'];
		...
		if ($this->wfstatus=='PA' || $this->wfstatus=='PH') {
			...
			if ($wf->initReadOnlyProcess('OPRPT2',$this->id,$this->lcd)) {
	...
	public function submit()
	{
		...
		try {
			...
			$this->updateDocman($connection,'OPERB1');
			$this->updateDocman($connection,'OPERB2');
			$this->updateDocman($connection,'OPERB3');
