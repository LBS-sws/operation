...
class AcctTypeForm extends CFormModel
{
	...
	{
		$sql = "select * from opr_account_type where id=$index";
	...
	protected function saveAcctType(&$connection)
	{
		...
		switch ($this->scenario) {
			case 'delete':
				$sql = "delete from opr_account_type where id = :id";
				break;
			case 'new':
				$sql = "insert into opr_account_type(
						acct_type_desc, rpt_cat, luu, lcu) values (
						:acct_type_desc, :rpt_cat, :luu, :lcu)";
				break;
			case 'edit':
				$sql = "update opr_account_type set 
	...
	public function isOccupied($index) {
		$rtn = false;
		$sql = "select a.id from opr_account a where a.acct_type_id=".$index." limit 1";
		$rows = Yii::app()->db->createCommand($sql)->queryAll();
		foreach ($rows as $row) {
			$rtn = true;
			break;
