...
class GoodsDoForm extends CFormModel
{
	...
	{
		return array(
			array('id, goods_code, name, img_url, classify_id, type, unit, price, rules_id, multiple, big_num, small_num, stickies_id, origin','safe'),
    ...
    public function downExcel(){
        ...
        ,'参考售价',"标签","数量倍率","最大数量","最小数量","混合規則");
        $rs = Yii::app()->db->createCommand()->select("a.*,f.name as stickies_name,b.name as classify_name,c.name as rules_name")
            ->from("opr_goods_do a")
            ->leftJoin("opr_classify b","a.classify_id=b.id")
            ->leftJoin("opr_stickies f","a.stickies_id=f.id")
        ...
        if($rs){
            foreach ($rs as $row){
                $list["body"][]=array(
                    ...
                    "unit"=>$row["unit"],
                    "price"=>$row["price"],
                    "stickies_name"=>$row["stickies_name"],
	...
	protected function saveGoods(&$connection) {
        ...
        GoodsImForm::uploadImg(get_class($this),$this->id,"opr_goods_do");
		return true;
	}
}
