...
class RankingHalfList extends CListPageModel
{
    public $year;
    public $month;
    public $allCity=0;
	...
	public function retrieveDataByPage($pageNum=1,$bool=true)
	{
        ...
        $monthScope =self::quarterHalf($this->month);
		...
		if (count($records) > 0) {
			foreach ($records as $k=>$record) {
                $this->attr[] = array(
                    ...
                    'rank_month'=>self::getHalfList($this->month,true),
		...
		if($bool){
            ...
            $session['rankingHalf_c01'] = $this->getCriteria();
        }
		return true;
	}

	public static function quarterHalf(&$month){
    ...
    public static function getHalfList($key=0,$bool=false){
	    $list = array(
	        1=>"1-6".Yii::t("rank","month unit"),
	        7=>"7-12".Yii::t("rank","month unit")
        );
	    if($bool){
