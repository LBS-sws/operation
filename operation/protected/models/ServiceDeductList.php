...
class ServiceDeductList extends CListPageModel
{
    public $year;
    public $month;
	...
	public function retrieveDataByPage($pageNum=1)
	{
		...
		if (count($records) > 0) {
			foreach ($records as $k=>$record) {
					$this->attr[] = array(
						...
						'deduct_date'=>CGeneral::toDate($record['deduct_date']),
						'deduct_type'=>self::getDeductType($record['deduct_type'],true),
		...
		$session['serviceDeduct_c01'] = $this->getCriteria();
		return true;
	}

	public static function getDeductType($key=0,$bool=false){
	    $list = array(
