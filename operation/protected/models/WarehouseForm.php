...
//
	public function validateName($attribute, $params){
        ...
        $rows = Yii::app()->db->createCommand()->select("id")->from("opr_warehouse")
            ->where('name=:name and id!=:id and city = :city', array(':name'=>$this->name,':id'=>$id,':city'=>$city))->queryRow();
        if($rows){
    ...
    public function deleteValidate(){
        $rs = Yii::app()->db->createCommand()->select()->from("opr_order_goods")->where('goods_id=:goods_id',array(':goods_id'=>$this->id))->queryRow();
        if($rs){
            return false;
        }else{
            return true;
    ...
    public function getPriceHistory($id){
        ...
        if(empty($html)){
            $html.="<tr><td colspan='4'><span>该物品没有单价历史</span></td></tr>";
        }
    ...
    public function getStorageHistory($id){
        ...
        if($rs){
            foreach ($rs as $row){
                $row["code"] = empty($row["code"])?"初始库存":$row["code"];
                $row["apply_time"] = empty($row["apply_time"])?date("Y-m-d",strtotime($row["lcd"])):$row["apply_time"];
                $html.="<tr>";
                if(empty($row["id"])){
                    $html.="<td>".$row["code"]."</td>";
                }else{
                    $html.="<td><a target='_blank' href='".Yii::app()->createUrl('storage/edit',array("index"=>$row["id"]))."'>".$row["code"]."</a></td>";
                }
                $html.="<td>".$row["apply_time"]."</td>";
                $html.="<td>".$row["goods_code"]."</td>";
                $html.="<td>".$row["name"]."</td>";
        ...
        if(empty($html)){
            $html.="<tr><td colspan='5'><span>该物品没有入庫記錄</span></td></tr>";
        }
        return array('status'=>1,'html'=>$html);
    }
        ...
        switch ($this->scenario) {
            ...
            case 'new':
                $sql = "insert into opr_warehouse(
							...
							:name, :unit, 0, :classify_id, :lcu, :goods_code,:city,:costing,:decimal_num,:min_num,:z_index
						)";
                break;
            case 'edit':
