...
class RankingYearList extends CListPageModel
{
    public $year;
    public $month;
    public $allCity=0;
	...
	public function retrieveDataByPage($pageNum=1,$bool=true)
	{
        ...
        $sqlText=Yii::app()->db->createCommand()
            ->select("a.employee_id,max(a.two_num) as two_num,max(a.pin_num) as pin_num,IFNULL(max(a.review_num),0) as review_num_a,IFNULL(min(a.review_num),0) as review_num_b,sum(a.other_score) as other_score,b.code,b.name,f.name as city_name")
		...
		$sql1 = "select staff.*,(staff.two_num+staff.pin_num+((staff.review_num_a+staff.review_num_b)/2)+staff.other_score) as score_sum 
				from ($sqlText) staff  
			";
